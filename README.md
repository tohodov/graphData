...
слотом, endpoint'ом или частью relation-подграфа, это должно быть выражено связями графа, типизированной
...
пользовательские данные, `GraphSearchService` ищет по ним по прямому запросу пользователя, а
`GraphStorageInitializer` записывает/сохраняет display-defaults для UI. Внутренние состояния, включая marker'ы
инициализации, должны выражаться узлами и связями графа, а не ключами в attributes.
